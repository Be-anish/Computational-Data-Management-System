...
<?php if (isset($_POST['addeditem'])) {
  ...
  if($writeoff>$quantity_received){
    $writeoff=$quantity_received;
  }
...
<?php
if (isset($_POST["writtenoffitem"])) {
  $id=$_POST['radio'];
  $flag=0;
  $sql1="select * from computational_items where id='{$id}';";
  $dat=mysqli_query($connection,$sql1);
  $ar=mysqli_fetch_assoc($dat);
  $sql2="select id as p_id from {$ar['item_type']} where {$ar['item_type']}_model={$id};";
  $p=mysqli_query($connection,$sql2);
  $sql3="delete from {$ar['item_type']} where {$ar['item_type']}_model={$id};";
  $t=mysqli_query($connection,$sql3);
  while($pr=mysqli_fetch_assoc($p))
  {$sql4="UPDATE `no_items_assigned` SET `{$ar['item_type']}`={$ar['item_type']}-1 where id='{$pr['p_id']}';";
  $na=mysqli_query($connection,$sql4);
  $sql5="UPDATE `lab` SET `{$ar['item_type']}`={$ar['item_type']}-1 where lab_no='{$pr['p_id']}';";
  $na2=mysqli_query($connection,$sql5);
}
$sql6="UPDATE `computational_items` SET `balance_instock`='0',`quantity_issued`='0',`writeoff`='{$ar['quantity_received']}' where id={$id};";
$upd_comp=mysqli_query($connection,$sql6);
if($upd_comp)$flag++;
if($flag){
  $_SESSION["successMessage"] = "Written Off Successfully!";
  redirect_to("admin.php");
} else {
  $_SESSION["error"] = "Couldn't be Written Off!";
  redirect_to("admin.php");
}
