...
<body>
  ...
  <div style="display:flex; flex-direction:col;">
    <div style="height: 100%; width:20%; float:left; top: 0; z-index: 1; left: 0; padding-top:0.5%; overflow-x:auto; overflow-y:auto; position:sticky;">
      <form class="nav flex-column" action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method='get'>
        ...
        <input class="btn btn-primary btn-outline-light mb-2" type="submit" name="writeoffitem" value="Writeoff computational items" style='height:7vh;'>
      </form>
    </div>
    <?php
    echo successMessage();
    ...
    <div class='form' style="display:flex; overflow-x:auto; width:80%; justify-content:center; margin:2% auto;">
      ...
      <?php if (isset($_GET['addfaculty'])) { ?>
        ...
        <form method='post' action="db_ops.php">
          ...
          <div><label for='resigned'>Resigned:</label><select class='form-select-sm' name='resigned' required>
...
// }
      ...
      <?php } ?>
      <?php if (isset($_GET['writeoffitem'])) { ?>
        <style>
        ...
        <form class="" action="db_ops.php" method="post">
        <input type="submit" name="writtenoffitem" value="Writeoff items" style="width:10%; margin-left:7%; margin-top:0;"><br />
        <table id="tableId">
            <tr class="thead">
                <th></th>
                <th>ID</th>
                ...
                <th>Wrte Off Quantity</th>
            </tr>
            <?php 
            $sql="select * from computational_items;";
            $list=mysqli_query($connection,$sql);
            while ($row = mysqli_fetch_assoc($list)) {
                $output = "<tr id='row_{$row['id']}' onclick='hover_click(this.id)'><td><input name='radio'  id='{$row['id']}' type='radio' value='{$row['id']}'/></td>";
