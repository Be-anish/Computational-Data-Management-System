...
<body>
  ...
  <div class="container">
        ...
        if(mysqli_num_rows($query))
          {echo "<table id='tableId' style='margin-top:2%;  margin-bottom:3%;'>
          <tr class='thead'>
            <th>#</th>
            <th>ID</th>
            <th>Name</th>
        ...
        if ($query) {
          ...
          <tbody>";
            ...
            echo "<h4 style='text-align: center; margin-top:3px; color:blue'>No {$item_type} allocated to staff...</h4>";
          }}
          while ($row = mysqli_fetch_assoc($query)) {
            $id = $row["id"];
            $dept_name = $row["dept_name"];
  ...
  <div class="container">
        ...
        $sql = "select DISTINCT lab.lab_no,lab.room_no,{$item_type}.quantity from lab,{$item_type} where lab.lab_no={$item_type}.id and {$item_type}.{$item_type}_model in (select computational_items.id from computational_items where Model = '$Model');";
        $query = mysqli_query($connection, $sql);
        ...
        if ($query) {
          if(mysqli_num_rows($query)){
            ...
            echo "<h4 style='text-align: center; margin-top:3px; color:blue'>No {$item_type} allocated to Labs...</h4>";
          }
          while ($row = mysqli_fetch_assoc($query)) {
            $lab_no = $row["lab_no"];
            $room_no = $row["room_no"];
